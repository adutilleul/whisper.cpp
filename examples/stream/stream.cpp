...
int main(int argc, char ** argv) {
    ...
    VadIterator small_vad("silero_vad.onnx", WHISPER_SAMPLE_RATE, VAD_TEST_FRAME_MS, params.vad_thold);
    ...
    while (is_running) {
        ...
        if (!use_vad) {
            ...
            audio.get(VAD_WINDOW_SIZE, pcmf32_new);

            const int test_samples  = VAD_TEST_FRAME_MS * WHISPER_SAMPLE_RATE / 1000;
            std::vector<float> inferences;
            for (int j = 0; j < VAD_WINDOW_SIZE; j += test_samples)
            {
                std::vector<float> r{&pcmf32_new[0] + j, &pcmf32_new[0] + j + test_samples};
                small_vad.reset_states();
                small_vad.predict(r);
                inferences.push_back(small_vad.get_output());
            }
